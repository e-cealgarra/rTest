...
# Load data
# ======================================================================================
load("C:/Users/mrrodriguezn/Projects/rpc-times-analysis/2017_2018_Normal_PLOP.RData")
DB <- data.table(DB)
# View(t(head(DB)))
...
DB[, planllegada := as.POSIXct(paste(Deliv.Date, Time), format = "%d.%m.%Y %H:%M:%S")]
...
dt <- DB[, .(Plnt, Deliv.Date, ZoneText, ClusterTe, OrderSale, Veh.Plate1, Driver, GRqty,
             TdC = as.numeric(TdC), planllegada, INSI, puntual,
             puntual30, puntual15)]
...
# Only big cities
cities <- daily[, .(volume = sum(volume), puntual = mean(punctuality30, na.rm = T)), by = ClusterTe][order(-volume)]
...
# Season of Year
daily[, season := month]
daily[month %in% c('August','September','October','November'), season := '2sem']
daily[month %in% c('February','March','April','May'), season := '1sem']
daily[month %in% c('June','July'), season :='summerbr']
daily[month %in% c('December','January'), season :='winterbr']
...
punt15[, historic := shift_left(runMean(punctuality15, min(21, .N)), 1), by = Plnt]
...
# Hot encoding
punt15 <- data.table(dummy.data.frame(punt15, names = c("hourGroup", "quarter", "weekday", "month", "season"), sep="_"))
...
# Plot
ggplot(dt_test, aes(x = nDeliveries/nDrivers, y = punctuality30, color = ClusterTe, size = nDeliveries)) +
...
lmodel <- lm(punctuality15 ~ hourGroup_day + hourGroup_night + nDeliveries +
               ...
               weekday_Fri + weekday_Sat + month_April + month_August + month_February +
               month_January + month_July + month_June + month_March + month_May +
               month_September + LoadCapacity + historic, data = dt_train)
